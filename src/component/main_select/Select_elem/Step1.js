...
import Lg_Select from './Step1_elem/Lg_Select';
import Sm_Select from './Step1_elem/Sm_Select';
import Step1_Submit from './Step1_elem/Step1_Submit';
...
class Step1 extends Component {
  ...
  render() {
    ...
    return (
      <div
        style={toStep2 ? { display: 'none' } : { display: 'flex' }}
        className='all-select-counrey-wrap'>
        {
          lang == 0 ?
            <div className={`select-country-head lang-${lang}`}>
              <span>{country_text['select_title'][lang][0]}</span>
              <span>{country_text['select_title'][lang][1]}</span>
            </div>
            :
            <div className={`select-country-head lang-${lang}`}>
              {country_text['select_title'][lang].split('').map((el, index) =>
                <span key={index}>
                  {el}
                </span>
              )}
